...
router.delete('/deletearea',async (req,res)=>{
    ...
    Area.findOneAndDelete({areaId},(resp)=>{
        return res.status(200).json({message:`area ${areaId} has been successfully deleted`});
    })
    .catch((err)=>{
...
router.post('/createarea',async (req,res)=>{
   ...
   const found = await Area.findOne({areaId})
    ...
    newArea.save()
    .then((resp)=>{
        return res.status(200).json({message:"area is successfully registered"});
    })
    .catch((err)=>{
...
router.get('/getallinfo',async(req,res)=>{
    Area.find({}).populate('campaignId').exec((err,docs)=>{
        if(err){
            return res.status(400).json({message:err});
        }else{
