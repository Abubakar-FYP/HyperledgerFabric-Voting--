...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from bluL1 and bluL2"
